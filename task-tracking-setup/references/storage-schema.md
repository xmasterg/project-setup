...
Humans normally use only `task_tracking/`, `ideation/`, and `open_task_board.html`. The HTML shell and its classic CSS/JavaScript are managed tracker assets. `render_tasks.py` validates task storage and atomically replaces only `task_board.data.js`; it leaves the shell, application, styling, and `tracker.json` unchanged. The board opens directly with `file://` and does not fetch data or require a server, package manager, framework, or build step. `setup/` is tracker-owned machinery.

Transactional install state, receipts, locks, journals, backups, and conflict candidates live outside the tracker tree under `.agents/project_management/setup/`. Unknown files in legacy roots are preserved; migration retires only exact schema-owned files recorded in the transaction.
...
Every JSON task file uses `{ "tasks": [] }`. `.agents/project_management/tasks/setup/tracker.json` contains tracker schema and project configuration. `task_board.data.js` uses `board_data_version: 1`, declares tracker schema version `4`, and keeps active and archived records separate.
...
Installer supports v1 `.agents/tasks.json`, v2 `.agents/tasks/`, and v3 root `tasks/`. Missing `urgency` becomes `normal`; missing `planning_docs` becomes `[]`. Unknown task fields and active/archive envelope fields remain on their records. When a v1 envelope has no tasks, its unknown fields are preserved under `tracker.json` → `migration_metadata.legacy_v1_empty_envelope`; unknown v2 index fields use `migration_metadata.legacy_v2_index`; unknown v3 tracker fields use `migration_metadata.legacy_v3_tracker`. A conflicting existing namespace aborts migration. During v3 migration, ideation files move into `.agents/project_management/tasks/ideation/` and task links are rebased to that project-relative path. Existing weekly archives retain their folder date. Old tracker-owned paths are removed only after v4 validation and board rendering succeed; unrelated `.agents` content remains.
